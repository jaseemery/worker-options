...
// ===== ALTERNATIVE: SIMPLE RECURRING WORKFLOW PATTERN =====

// If you prefer a simpler approach without Schedules, 
// here's a self-recurring workflow pattern:

[Workflow]
public class SimpleRecurringWorkflow
{
    private const int MaxIterations = 1000; // Prevent infinite history growth
    private int _iterationCount = 0;

    [WorkflowRun]
    public async Task RunAsync(RecurringWorkflowInput input)
    {
        var logger = Workflow.Logger;
        logger.LogInformation("Starting iteration {Count} of recurring workflow", _iterationCount + 1);
        ...
        while (_iterationCount < MaxIterations)
        {
            try
            {
                ...
                _iterationCount++;
                logger.LogInformation("Completed iteration {Count}", _iterationCount);

                // Wait before next iteration (30 seconds)
                await Workflow.DelayAsync(TimeSpan.FromSeconds(30));

                // Use continue-as-new periodically to prevent history growth
                if (_iterationCount >= 100 || Workflow.ContinueAsNewSuggested)
                {
                    logger.LogInformation("Using continue-as-new after {Count} iterations", _iterationCount);
                    throw Workflow.CreateContinueAsNewException(
                        (SimpleRecurringWorkflow wf) => wf.RunAsync(input));
                }
            }
            catch (Exception ex) when (!(ex is ContinueAsNewException))
            {
                logger.LogError(ex, "Error in recurring workflow iteration {Count}", _iterationCount);
